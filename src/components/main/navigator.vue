...
function getPathHandler(theme?: string) {
  switch (theme) {
    ...
    case 'tianpan-compact':
      return function (nodePathData: any[], x: number, y: number, width: number) {
        // eslint-disable-next-line no-bitwise
        const r = width >> 1;
        nodePathData.push('M', x, y + r, 'a', r, r, 0, 1, 1, 0, 0.01, 'z');
      };
    default: {
      return function (nodePathData: any[], x: number, y: number, width: number, height: number) {
...
onMounted(() => {
  nextTick(() => {
    ...
    // 主题切换事件
    minder.on('themechange', function (e: any) {
