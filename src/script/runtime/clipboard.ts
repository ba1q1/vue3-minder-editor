...
interface IMimeType {
  getMimeTypeProtocol(mimeType: string): Function;
  isPureText(mimeType?: string): boolean;
  whichMimeType(mimeType?: string): string;
  getPureText(data?: string): string;
}

interface IData {
  getRegisterProtocol(protocol: string): { encode: Function; decode: Function };
}
